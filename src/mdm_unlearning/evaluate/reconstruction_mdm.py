...
def run_eu_unlearning(model, data_dir, corpus, seq_len,
                      max_steps=10000, lr=5e-5, eu_lambda=1.0, device='cuda'):
    """Run Exclusive Unlearning: uniform CE on forget + NLL on retain."""
    ...
    # Retain dataloader (all corpora except forget)
    retain_files = []
    ...
    step = 0
    for data in tqdm(forget_dl, total=max_steps, desc=f"EU-Unlearning {corpus}"):
        ...
        # Forget loss: drive predictions toward uniform distribution
        V = logits.size(-1)
        uniform = torch.ones(V, device=device) / V
        forget_loss = F.kl_div(
            F.log_softmax(logits[mask_indices], dim=-1),
            uniform.unsqueeze(0).expand(mask_indices.sum(), -1),
            reduction='batchmean')

        # Retain loss: standard ELBO on other corpora
        try:
    ...
    for data in tqdm(forget_dl, total=max_steps, desc=f"Unlearning {corpus}"):
        ...
        if step % meta_every == 0:
            ...
            for _ in range(meta_k):
                ...
                r_n, r_m, r_p = forward_process(r_ids)
                with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                    r_log = model(r_n)
            ...
            f_n, f_m, f_p = forward_process(input_ids)
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                f_log = model(f_n)
...
def main():
    ...
    if args.unlearn_method == 'kl':
        ...
        model = run_eu_unlearning(
            model, args.data_dir, args.unlearn_corpus, args.seq_len,
            max_steps=args.unlearn_steps, eu_lambda=args.eu_lambda,
            device=args.device)
