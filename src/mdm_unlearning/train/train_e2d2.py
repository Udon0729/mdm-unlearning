"""
E2D2-style Encoder-Decoder MDM training for UnTrac experiments.
Based on train_mdm_untrac.py, modified for:
- TransEncoderDecoder model with cross-attention bottleneck
- Encoder sees clean tokens, decoder sees noisy tokens
- Same training setup: 40K steps, batch=8, lr=5e-5, 8 corpora
"""
import glob
import math
import sys
...
from mdm_unlearning.models.diffmodel import Block, Config
from mdm_unlearning.models.enc_dec_diffmodel import TransEncoderDecoder, EncoderBlock, DecoderBlock, forward_process_block
...
def setup(
    ...
    hp_name = f'e2d2-untrac-{args.model}M-{args.max_steps}steps{exclude_tag}'
    ...
    if devices > 1:
        strategy = FSDPStrategy(
            auto_wrap_policy={EncoderBlock, DecoderBlock},
...
def main(fabric, train_data_dir, val_data_dir, resume):
    ...
    with fabric.init_module(empty_init=False):
        model = TransEncoderDecoder(config)
        n_total = config.n_encoder_layers + config.n_decoder_layers
        model.apply(partial(model._init_weights, n_layer=n_total))
...
def train(fabric, state, train_dataloader, val_dataloader, monitor, resume):
    ...
    # Skip FLOP estimation for encoder-decoder (different architecture)
    estimated_flops = 0
    fabric.print(f"E2D2: {model.n_encoder_layers} enc + {model.n_decoder_layers} dec layers")
    ...
    for train_data in train_dataloader:
        ...
        diff_bs = getattr(model, 'diffusion_block_size', 128)
        noisy_input, mask_indices, p_mask = forward_process_block(input_ids, block_size=diff_bs)
